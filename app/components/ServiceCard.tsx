    ...
    >
      ...
      <div className="relative">
        <div className="flex items-center gap-3 mb-3">
          {icon && <div className="text-accent text-3xl flex-shrink-0">{icon}</div>}
          <h3 className="text-2xl font-semibold text-white tracking-tight">{title}</h3>
        </div>
        {summary && <p className="text-sm text-slate-200 font-medium mb-3 leading-snug">{summary}</p>}
        {description && <p className="text-base text-slate-300 leading-7 mb-6">{description}</p>}
      </div>
      <div className="mt-auto pt-4">
        <Link href={href} className="inline-flex items-center justify-center rounded-full bg-accent px-4 py-3 text-sm font-semibold text-slate-950 transition hover:bg-accent/90 w-full text-center">
          Learn More
        </Link>
      </div>
    </div>
